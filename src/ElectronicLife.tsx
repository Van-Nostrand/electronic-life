...
import { ICritter, IBouncingCritter } from './types'

const initdata = (function () {
  const critterArray: Array<ICritter | IBouncingCritter> = []
  const worldMap = DEFAULT_PLAN.map((row, y) => {
    return row.split('').map((tile, x) => {
      if (tile === 'b') {
        critterArray.push(
          new BouncingCritter({ x, y, facing: { x: 0, y: -1 } })
        )
        return ' '
      }
      else if (tile === 'w') {
        critterArray.push(
          new WallFollower({ x, y, facing: { x: 0, y: -1 } })
        )
        return ' '
      }
...
export default function ElectronicLife () {
  ...
  useEffect(() => {
    const takeTurn = () => {
      const newCritters: Array<ICritter> = critters.map(critter => critter.takeTurn(world))
