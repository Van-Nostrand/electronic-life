...
export const updateCreatures = (creatureArray: any, worldArray: any) => {
  ...
  creatureArray = creatureArray.map((creature: any) => {

    let hasChosenDirection = false
    //while the creature has not moved yet
    while (!hasChosenDirection) {
      const facedCell = worldArray[creature.y + creature.facing.y][creature.x + creature.facing.x]
