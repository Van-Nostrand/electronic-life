import { ICritter, TCoordinates, IRelativeCoordinates, IWallFollower } from '@/types'
///////////////
...
export const faceNewRandomDirection = (oldDirection: TCoordinates): TCoordinates => {
  let newDirectionIsGood = false
  const newDirection: TCoordinates = { x: 0, y: 0 }
  do {
    newDirection.x = Math.floor(Math.random()*3) - 1
    newDirection.y = Math.floor(Math.random()*3) - 1
    // if the new direction does not equal the old direction and the critter isn't just staring at their feet
    if (newDirection.x !== oldDirection.x && newDirection.y !== oldDirection.y && !(newDirection.x === 0 && newDirection.y === 0)) newDirectionIsGood = true
  } while (!newDirectionIsGood)
...
export const findNearestWall = (critter: IWallFollower, worldMap: string[][]): IRelativeCoordinates => {
...
export const scanSurroundingsForItem = (
  ...
  let counterClockwiseCoordinates, clockwiseCoordinates: TCoordinates
...
export const findNextSpaceToMoveAlongWall = (worldMap: Array<Array<string>>, wallFollower: IWallFollower): [ TCoordinates, IRelativeCoordinates ] => {
  const { x, y, wallCoordinate, movesClockwise } = wallFollower
...
export const checkBorderOf2dArray = (
  ...
  let clockwiseCoordinates: TCoordinates | Error = { x: startX, y: startY }
  const directionFound = false
  do {
...
export const getRelativeCCwCoordinate = (x: number, y: number, upperBounds: {x: number; y: number;}, lowerBounds: {x: number; y: number;}): TCoordinates => {
  switch (true) {
...
export const getRelativeCwCoordinate = (x: number, y: number, upperBounds: {x: number; y: number;}, lowerBounds: {x: number; y: number;}): TCoordinates => {
  switch (true) {
...
export const getCounterClockwiseCoordinate = (x: number, y: number, arrayLength: number): TCoordinates => {
  switch (true) {
...
export const getClockwiseCoordinate = (x: number, y: number, arrayLength: number): TCoordinates => {
  switch (true) {
...
export const moveClockwiseAroundCoordinate = (facing: TCoordinates): TCoordinates => {
  switch (true) {
...
export const moveCounterClockwiseAroundCoordinate = (facing: TCoordinates): TCoordinates => {
  switch (true) {
    case facing.y === 1 && facing.x < 1:
      return ({ x: -1, y: 0 }) // move left
    case facing.x === 1 && facing.y > -1:
...
export const deriveDirectionFromCoordinates = (
  targetCoordinates: TCoordinates,
